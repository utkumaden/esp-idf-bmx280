...
#include <stdint.h>
#include <limits.h>
#include <freertos/FreeRTOS.h> //For vTaskDelay to delay after sensor reset
#include "sdkconfig.h"

#define USE_I2C_MASTER_DRIVER 1

/**
 * The ESP-IDF version 5.3.1 guide state that the legacy i2c.h driver and the i2c_master.h driver CANNOT coexist
 */
...
#include "bmx280_bits.h"
    ...
    /**
     * Probe for the senors and read calibration data.
     * @param bmx280 Driver structure.
     * @param dev_handle I2C device handle for BMX280 device, expected to be initialized by caller.
     */
    BMXAPI esp_err_t bmx280_init(bmx280_t *bmx280, i2c_master_dev_handle_t dev_handle);
#else
/**
