...
#if USE_I2C_MASTER_DRIVER
    err = i2c_master_transmit(bmx280->dev_handle, &addr, 1, -1);
    if (err != ESP_OK)
    {
        ESP_LOGE("bmx280", "Error transmitting I2C data");
        return err;
    }
    err = i2c_master_receive(bmx280->dev_handle, dout, size, -1);
    return err;
#else
...
static esp_err_t bmx280_write(bmx280_t *bmx280, uint8_t addr, const uint8_t *din, size_t size)
{
    ...
    buffer[0] = addr;
    memcpy(buffer + 1, din, size);
    esp_err_t err = i2c_master_transmit(bmx280->dev_handle, buffer, size + 1, -1);
    free(buffer);
    return err;
#else
    esp_err_t err;
...
{
#if USE_I2C_MASTER_DRIVER
    ESP_LOGI("bmx280", "Probing for BMP280/BME280 sensors on I2C bus");
#else
...
#if USE_I2C_MASTER_DRIVER
esp_err_t bmx280_init(bmx280_t *bmx280, i2c_master_dev_handle_t dev_handle)
{
    ESP_LOGI("bmx280", "Starting BMX280 initialization");
    if (bmx280 == NULL)
        return ESP_ERR_INVALID_ARG;

    bmx280->dev_handle = dev_handle;
